...
const login = async () => {
  ...
  try {
    ...
    if (err.response) {
      errorMessage.value = err.response.data?.message || 'เข้าสู่ระบบล้มเหลว';
    } else if (err.request) {
...
.error-message {
  margin-top:10px; padding:10px; background:#f8d7da; color:#721c24;
  border:1px solid #f5c6cb; border-radius:4px; text-align:center;
}
</style>
